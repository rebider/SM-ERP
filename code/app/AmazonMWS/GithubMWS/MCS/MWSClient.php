...
use App\Models\AmazonApiServiceResponse;
...
class MWSClient{
    ...
    /**
     * 返回基于ASIN、GCID、SellerSKU、UPC、EAN、ISBN和JAN值的列表的产品及其属性。
	 ...
	 */
	public function deleteProductBySKU(array $deleteProducts) {
     ...
     * @return array feed submission result
     */
    public function updateStock(array $productStock)
    {
        $feed = [
        ...
        $i = 1;
        foreach ($productStock as $sku => $quantity) {
            $feed['Message'][] = [
                'MessageID' => $i,
     ...
     * $standardprice包含sku作为键和price作为值的数组
     * @param array $standardprice an array containing sku as key and price as value
     * $salesprice一个可选数组，其中包含一个数组，其中包含SalePrice、StartDate和EndDate的键/值对 DateTime对象中的日期
     * Price必须格式化为XSD数值数据类型(http://www.w3schools.com/xml/schema_dtypes_numeric.asp)
    ...
    public function updatePrice(array $standardprice, array $saleprice = null) {
        ...
        foreach ($standardprice ['standardprice'] as $sku => $price) {
            $feed['Message'][] = [
                'MessageID' => $i,
                'Price' => [
                    'SKU' => $sku,
            ...
            if (isset($standardprice ['saleprice'])) {
                $saleprice = $standardprice ['saleprice'];
                if (isset($saleprice[$sku]) && is_array($saleprice[$sku])) {
                    $StartDate = new \DateTime($saleprice[$sku]['StartDate']);
                    $EndDate = new \DateTime($saleprice[$sku]['EndDate']);
    ...
    /**
     * @param array $standardImage
     * @return array
     * Note: 图片上传
     * Data: 2019/6/14 21:11
     * Author: zt7785
     */
    public function updateGoodsImage(array $standardImage) {
        ...
        '270002_1'=>[
        'type'=>'Main',
        'url'=>'http://jinrong.zongteng.net/fkweb/images/banner.jpg',
        ],
        '270002_2'=>[
        'type'=>'Alternate',
        'url'=>'http://jinrong.zongteng.net/fkweb/images/banner1.jpg',
        ],
        '270002_3'=>[
        'type'=>'Alternate',
        'url'=>'http://jinrong.zongteng.net/fkweb/images/banner4.jpg',
        ],
        ];
         */
        //The type of image (Main, Alternate, or Swatch)
        foreach ($standardImage ['standardImage'] as $skuKey => $image) {
            $sku = explode('_',$skuKey)[0];
            $feed['Message'][] = [
                'MessageID' => $i,
                'OperationType' => 'Update',
                'ProductImage' => [
...
//                    'ImageType' => 'Main',
     ...
     * @param  object $MWSProduct or array of Custom objects
     * @return array
     */
    public function postProducts($MWSProduct, $template = 'Custom', $version = null, $signature = null)
    {
    ...
    public function postProduct($MWSProduct) {
        ...
        $csv->insertOne($header);
        $csv->insertOne($header);
//58494018060 一个header 1299 quantity = 2; 260001 B07PHMR9PW
//58495018060 两个header 3999 quantity = 20; 270002 B07PDJZHBD
        foreach ($MWSProduct as $product) {
            $csv->insertOne(
                array_values($product->toArray())
            );
    ...
    public function submitFeedShipping ($param) {
            ...
            foreach ($items ['item'] as $item) {
                $feed['Message'][] = [
                    ...
                    'OrderFulfillment' => [
                        ...
                        'FulfillmentData'=>[
                            'CarrierName'=>$items ['logistics_info'] ['carrier_name'],
                            'ShippingMethod'=>$items ['logistics_info'] ['plat_logistic_name'],
        ...
        return $this->SubmitFeed('_POST_ORDER_FULFILLMENT_DATA_', $feedContent);
    }

    /**
     * 返回提要处理报告和Content-MD5头。
	...
	$purgeAndReplace = isset($options['PurgeAndReplace']) ? $options['PurgeAndReplace'] : false;
        ...
        $shippingRequestRes = array_merge($response['SubmitFeedResult']['FeedSubmissionInfo'],['RequestId'=>$response ['ResponseMetadata'] ['RequestId']]);
        return $shippingRequestRes;
    }
    ...
    /**
     * 基于ASIN，为您自己的报价列表返回定价信息。基于ASIN，为多达20种产品的最低价格活跃报价列表返回定价信息。
    ...
    /**
     * 返回提交请求的卖家可以在其中销售的市场列表，以及包含该市场中卖家特定信息的参与者列表
