...
abstract class MarketplaceWebServiceOrders_Model
{
    ...
    protected $_nameSpace = '\App\AmazonMWS\MarketplaceWebServiceOrders';
    ...
    private function _fromDOMElement(\DOMElement $dom)
    {
        ...
        $needleStr = '_Model_';
        $needleSpace = DIRECTORY_SEPARATOR.'Model'.DIRECTORY_SEPARATOR;
            ...
            if (is_array($fieldType)) {
                if ($fieldType[0] == "object") {
                    ...
                    if ($elements->length >= 1) {
                        if (is_bool(strpos($fieldType[0],$needleStr))) {
                            $nameSpace = $this->_nameSpace;
                        } else {
                            $nameSpace = $this->_nameSpace.$needleSpace;
                        }
                        $fieldType[0] = $nameSpace.$fieldType[0];
                ...
                if ($this->_isComplexType($fieldType)) {
                    $elements = $xpath->query("./*[local-name()='$fieldName']", $dom);
                    if ($elements->length == 1) {
                        if (is_bool(strpos($fieldType,$needleStr))) {
                            $nameSpace = $this->_nameSpace;
                        } else {
                            $nameSpace = $this->_nameSpace.$needleSpace;
                        }
                        $fieldTypes = $nameSpace.$fieldType;
                        $this->_fields[$fieldName]['FieldValue'] = new $fieldTypes($elements->item(0));
                    }   
                } else {
                    if($fieldType[0] == "@") {
