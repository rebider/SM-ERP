...
class AmazonResponseTask extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'AmazonTask:AmazonResponse';
     ...
     * @var string
     */
    protected $description = '亚马逊响应任务';


    protected $task_name = 'AmazonResponseTask';
    ...
    public function handle()
    {
        ...
        $AmazonApiServiceResponse = new AmazonApiServiceResponse();
        $current_time = date('Y-m-d H:i:s');
        $search_time = date('Y-m-d H:i:s',strtotime('-2 day'));
        //两天之内 未处理的上传数据
        $feedInfos = $AmazonApiServiceResponse->with('SettingShops')->whereDate('created_at','>',$search_time)->where('is_finished',$AmazonApiServiceResponse::UN_FINISHED)->get();
//        $feedInfos = $AmazonApiServiceResponse->with('SettingShops')->whereDate('created_at','>',$search_time)->where('is_finished',$AmazonApiServiceResponse::UN_FINISHED)->where('id',50)->get();
        ...
        try {
            $AmazonApiServiceResponse->feedSubmitResultLogic($feedInfos,$current_time);
            $task_name = $this->task_name;
