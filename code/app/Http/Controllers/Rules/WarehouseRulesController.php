<?php
    ...
    use App\Models\RulesWarehouseCondition;
    ...
    use App\Validates\WarehouseAllocationConditionsValidate;
    ...
    class WarehouseRulesController extends Controller
    {
        public $menusModel = [];
         ...
         * Author: zt8076
         */
        public function warehouseAllocationIndex(Request $request)
        {
            //.layui-table-cell  去掉height: 28px;  嵌套一个div 绑定事件 事件删除该样式高度
            //便捷菜单
            $responseData ['shortcutMenus'] = $this->menusModel->getShortcutMenu(RulesWarehouseAllocation::RULES_ORDER_MENUS_ID);
            return view('Rules/warehouseAllocation/index')->with($responseData);
        }
         ...
         * Author: zt8076
         */
        public function warehouseAllocationSearch(Request $request)
        {
            $data = $request->all();
            ...
            $result = RulesWarehouseAllocation::getAllocationDatas($param, $offset, $limit);
         ...
         * Author: zt8076
         */
        public function warehouseAllocationDetail(Request $request, $id)
        {
            $datas = $request->all();
            if (empty($id)) {
                abort(404);
            ...
            //获取所有问题类型
            $responseData['data'] = RulesWarehouseAllocation::getWarehouseAllocationByOpt(['field' => 'id', 'value' => $id], $optionTrouble);
            ...
            $conditions = [];
            $warehouse_ids = [];
            if (!empty($responseData['data'] ['rules_warehouse_condition'])) {
                //condition id 做默认选中
                $rules_trouble_condition_arr = array_column($responseData['data'] ['rules_warehouse_condition'], 'condition_id');
                //条件id升序
                array_multisort(array_column($responseData['data'] ['rules_warehouse_condition'], 'condition_id'), SORT_ASC, $responseData['data'] ['rules_warehouse_condition']);
                $conditions = $responseData['data'] ['rules_warehouse_condition'];
            }
            if (!empty($responseData['data']['warehouse_ids'])) {
                if (strpos($responseData['data']['warehouse_ids'], ',')) {
                    $warehouse_ids = $responseData['data']['warehouse_ids'];
                    $warehouse_ids = explode(',', $warehouse_ids);
                } else {
                    $warehouse_ids = (array)$responseData['data']['warehouse_ids'];
                }
                //按照添加时顺序排序
                $sortId = implode(',',$warehouse_ids);
                $selectWarehouse = SettingWarehouse::whereIn('id', $warehouse_ids)->orderByRaw(DB::raw("FIELD(id,$sortId)"))->get()->toArray();
            }
            ...
            if (in_array(12, $rules_trouble_condition_arr)) {
                ...
                //仓库
                $warehouses = SettingWarehouse::where('user_id', $optionTrouble ['user_id'])->whereNotIn('id', $warehouse_ids)->get()->toArray();
            }
            ...
            if (empty($datas['edit'])) {
                ...
                $responseData ['selectWarehouse'] = $selectWarehouse;
                //查看
                //根据condition id 做选中
                //渲染ul标签
                return view('Rules/warehouseAllocation/detail')->with($responseData);
            } else if ($datas['edit'] == 1) {
                //编辑
                if ($request->isMethod('post')) {
                    //编辑数据提交
                    //数据处理
                    $troubleConditionDatas = RulesWarehouseCondition::troubleDatasLogics($datas, $conditionDatas, $optionTrouble);
                    $validator = Validator::make(
                        $troubleConditionDatas['troubleData'],
                        WarehouseAllocationConditionsValidate::getAllocationConditionsRules(false, $id),
                        WarehouseAllocationConditionsValidate::getAllocationConditionsMessages(),
                        WarehouseAllocationConditionsValidate::getAllocationConditionsAttributes()
                    );
                    if ($validator->fails()) {
                    ...
                    DB::beginTransaction();
                    try {
                        $ruluRe = RulesWarehouseAllocation::postGoods($id, $troubleConditionDatas['troubleData']);
                        if ($ruluRe) {
                            foreach ($troubleConditionDatas ['conditionData'] as $condVal) {
                                ...
                                if (in_array($condVal ['condition_id'], $responseData ['conditionIds'])) {
                                    ...
                                    RulesWarehouseCondition::postGoods($countryIds['id'], $condVal);
                                    unset($responseData ['conditionIds'] [array_search($condVal ['condition_id'], $responseData ['conditionIds'])]);
                                } else {
                                    //新增
                                    RulesWarehouseCondition::postGoods(0, $condVal);
                                }
                            }
                            //编辑取消的条件处理
                            if (!empty($responseData ['conditionIds'])) {
                                foreach ($responseData ['conditionIds'] as $cancleId) {
                                    ...
                                    //取消状态
                                    $cancleData ['is_used'] = RulesWarehouseCondition::STATUS_CLOSEDING;
                                    //当前客户 操作人
                                    $cancleData ['created_man'] = $currentUser->userId;
                                    RulesWarehouseCondition::postGoods($countryIds['id'], $cancleData);
                                }
                            }
                        } else {
                            DB::rollback();
                        ...
                        DB::rollback();
                        Common::mongoLog($e,'仓库规则','仓库规则修改',__FUNCTION__);
                        return AjaxResponse::isFailure('修改规则失败');
                    } catch (\Error $e){
                        Common::mongoLog($e,'仓库规则','仓库规则修改',__FUNCTION__);
                    }
                    ...
                    $checkedData = RulesWarehouseCondition::getCheckedDatas($conditions);
                    $responseData ['checkedDatas'] = $checkedData;
                    $responseData ['warehouses'] = $warehouses;
                    $responseData ['selectWarehouse'] = $selectWarehouse;
                    return view('Rules/warehouseAllocation/add_edit')->with($responseData);
                }
            }
        }
         ...
         * Author: zt8076
         */
        public function warehouseAllocationCreated(Request $request)
        {
            //所有条件
            $param ['is_show'] = 1;
            ...
            if ($request->isMethod('get')) {
                ...
                return view('Rules/warehouseAllocation/warehouse_rules_dialog')->with(['rules' => $responseData, 'platforms' => $platforms, 'shops' => $shopsArr, 'countrys' => $countrys, 'attrs' => $attrArrs, 'warehouses' => $warehouses, 'logistics' => $logistics, 'currencyInfos' => $currencyInfos]);
            } else if ($request->isMethod('post')) {
                $datas = $request->all();
                //数据处理
                //S1. 规则校验
                //S2. 参数组装
                $troubleConditionDatas = RulesWarehouseCondition::troubleDatasLogics($datas, $conditionDatas, $userInfo);
                //数据验证
                $validator = Validator::make(
                    $troubleConditionDatas['troubleData'],
                    WarehouseAllocationConditionsValidate::getAllocationConditionsRules(true),
                    WarehouseAllocationConditionsValidate::getAllocationConditionsMessages(),
                    WarehouseAllocationConditionsValidate::getAllocationConditionsAttributes()
                );
                if ($validator->fails()) {
                ...
                DB::beginTransaction();
                try {
                    $ruluRe = RulesWarehouseAllocation::postGoods(0, $troubleConditionDatas['troubleData']);
                    if ($ruluRe) {
                        foreach ($troubleConditionDatas ['conditionData'] as $condVal) {
                            $condVal ['trouble_rule_id'] = $ruluRe->id;
                            RulesWarehouseCondition::postGoods(0, $condVal);
                    ...
                    DB::rollback();
                    Common::mongoLog($e,'仓库规则','仓库规则创建',__FUNCTION__);
                    return AjaxResponse::isFailure('添加规则失败');
                } catch (\Error $e){
                    Common::mongoLog($e,'仓库规则','仓库规则创建',__FUNCTION__);
                }
        ...
        public function delete($id)
        {
            ...
            $RulesWarehouseTrouble = RulesWarehouseAllocation::destroy($id);
            if ($RulesWarehouseTrouble) {
                RulesWarehouseCondition::where('trouble_rule_id', $id)->delete();
