 ...
 * Author: zt7785
 */
class OrdersRakuten extends Model
{
    protected $table = 'orders_rakuten';
     ...
     * @var 未匹配
     */
    const RAKUTEN_MAPPING_STATUS_UNFINISH = 1;
    /**
     * @var 已匹配
     */
    const RAKUTEN_MAPPING_STATUS_FINISHED = 2;
    /**
     * @var 匹配失败
     */
    const RAKUTEN_MAPPING_STATUS_FAIL = 3;
    ...
    {
        return self::where($option,$value)->first()->toArray();
    }
     ...
     * Note: 获取乐天订单状态信息
     * Data: 2019/5/21 16:35
     * Author: zt7785
     */
    public static function getRakutenOrdersInfoByCode($orderCode,$userId)
    {
        $result = self::where('orderNumber',$orderCode)->where('created_man',$userId)->first(['id','totalPrice','is_system','match_status','cw_code']);
        return $result;
    }
    ...
    public function orderDataLogics ($orderDatas ,$shopInfo = [] ,$is_delete = false) {
        ...
        foreach ($orderDatas as $key=>$value){
            ...
            if ($userLogisticsInfo->isEmpty ()) {
                $userLogisticsInfo = [];
            } else {
                $userLogisticsInfo = $userLogisticsInfo ->toArray();
            }
...
//                $this->postNewOrderInfo($sale_info ,$v ,$current_time ,$order_rakuten_source ,$shopIds ,$shopInfo ,$currencyInfo ,$country_id ,$country ,$userLogisticsCodes ,$userLogisticsInfo );
                    ...
                    $dingPushData ['task'] = 'Rakuten订单数据处理';
                    $dingPushData ['message'] = $exception_data ['file']."\n\n".$exception_data ['line']."\n\n".$exception_data ['msg'];
                    $exceptionDing ['path'] = 'orderDataLogics';
                    ...
                    if ($cw_erp_exist ['deliver_status'] == Orders::ORDER_DELIVER_STATUS_FILLED_PART) {
                        ...
                        if ($cw_erp_exist ['sales_status'] == Orders::ORDER_SALES_STATUS_INITIAL) {
                            ...
                            if ($unDeliverAmount > 0) {
                                ...
                                //V4取消订单日志
                                self::orderTsakLogs($cw_erp_exist,$current_time,OrdersLogs::LOGS_ORDERS_CANCEL,'接口同步订单检测平台订单已取消');
                                $is_obsoleted = true;
                            }
...
//        }
    ...
    public function postNewOrderInfos($sale_info ,$v ,$current_time ,$order_rakuten_source ,$shopIds ,$shopInfo ,$currencyInfo ,$country_id ,$country ,$userLogisticsCodes ,$userLogisticsInfo )
    {
        try {
            ...
            $orders_original_data ['order_number']      = $temp_ori_order['cw_code'];
            ...
            $orders_original_data ['addressee_name']    = $consignee_name;
            $orders_original_data ['addressee1']        = $temp_ori_order['send_subAddress'];
            $orders_original_data ['addressee2']        = '';
            ...
            $orders_original_data ['platform_order']    = $temp_ori_order['orderNumber'];
            ...
            //订单商品全部有映射关系才算匹配成功
            $skus               = array_column($v['PackageModelList'][0]['ItemModelList'], 'itemNumber');
//            $skus               = array_column($v['PackageModelList'][0]['ItemModelList'], 'itemId');
            $mappingInfo        = GoodsMapping::getMappingInfoByUseridSku($sale_info['user_id'], $skus, Platforms::RAKUTEN);
            $mappingInfoItemURL = array_column($mappingInfo, 'itemURL');
            if (empty($mappingInfo)) {
                $match_status = false;
            ...
            if ($match_status) {
                ...
                $erpOrderInfo ['logistics']             = $temp_ori_order['deliveryName'];
                //乐天订单有卡车物流方式
                $erpOrderInfo ['logistics_id']          = $orders_original_data ['logistics_id'];
            ...
            foreach ($v['PackageModelList'][0]['ItemModelList'] as $key => $rakuten_product) {
                ...
                $rakuten_products[$key]['created_at']                       = $rakuten_products[$key]['updated_at'] = $current_time;
                ...
                $orders_original_product_info[$key]['goods_img']            = '';
...
//                $orders_original_product_info[$key]['sku']                  = $rakuten_products[$key]['itemNumber'];
                $orders_original_product_info[$key]['sku']                  = $rakuten_products[$key]['itemId'];
                ...
                //匹配成功之后 应用该数据
                if ($match_status) {
                    $mappingKey =  array_search($rakuten_products[$key]['itemNumber'], $mappingInfoItemURL);
                    $goods_id = GoodsMappingGoods::getGoodsIdByMappingid($mappingInfo[$mappingKey]['id']);
                    $order_product_info[$key]['goods_id']                   = $goods_id['goods_id'];
                    ...
                    $order_product_info[$key]['created_man']                = $sale_info['user_id'];
                    $order_product_info[$key]['user_id']                    = $sale_info['user_id'];
                    $order_product_info[$key]['order_id']                   = $erpOrderId;
                    //原始表商品图片
                    $orders_original_product_info[$key]['goods_img']        = $goods_id ['goods'] ? $goods_id ['goods'] ['goods_pictures'] : '';
                    $order_product_info[$key]['order_type']                 = OrdersProducts::ORDERS_CWERP;
                    $order_product_info[$key]['product_name']               = $rakuten_products[$key]['itemName'];
                    $order_product_info[$key]['sku']                        = ($two_sku) ? $two_sku : $rakuten_products[$key]['itemNumber'];
                    $order_product_info[$key]['currency']                   = $currencyInfo ['currency_code'];
                    $order_product_info[$key]['buy_number']                 = $rakuten_products[$key]['units'];
                    $order_product_info[$key]['weight']                     = $goods_id['goods']? $goods_id['goods'] ['goods_weight']:'0.00';
                    $order_product_info[$key]['univalence']                 = $rakuten_products[$key]['price'];
                    $order_product_info[$key]['rate']                       = $currencyInfo ['rate'];
                    $order_product_info[$key]['is_deleted']                 = OrdersProducts::ORDERS_PRODUCT_UNDELETED;
                    $order_product_info[$key]['RMB']                        = bcmul(bcmul($currencyInfo ['rate'], $rakuten_products[$key]['price']), $rakuten_products[$key]['units']);
                    $order_product_info[$key]['created_at']                 = $order_product_info[$key]['updated_at'] = $current_time;
                    $order_product_info[$key]['already_stocked_number']     = $order_product_info[$key]['cargo_distribution_number'] = $order_product_info[$key]['delivery_number'] = $order_product_info[$key]['partial_refund_number'] = 0;
                }
            }
            ...
            $exception_data = [
                'start_time'                => $current_time,
                'orig_order_id_rakuten'     => $temp_order_id,
            ...
            $dingPushData ['task'] = 'Rakuten订单数据处理';
            $dingPushData ['message'] = $exception_data ['file']."\n\n".$exception_data ['line']."\n\n".$exception_data ['msg'];
            $exceptionDing ['path'] = 'postNewOrderInfos';
            DingRobotWarn::robot($exceptionDing,$dingPushData);
            LogHelper::info($exception_data,null,$exceptionDing ['type']);
        }
