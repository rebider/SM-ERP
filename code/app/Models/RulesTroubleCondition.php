 ...
 * Author: zt7785
 */
class RulesTroubleCondition extends Model
{
    protected $table = 'rules_trouble_condition';
     ...
     * Author: zt7785
     */
    public function RulesOrderTroubleCondition() {
        return $this->belongsTo(RulesOrderTroubleCondition::class, 'condition_id', 'id');
    }
    ...
    public static function troubleDatasLogics($datas,$conditionDatas,$userInfo) {
        ...
        $troubleField = ['trouble_rules_name','trouble_desc','trouble_type_id','opening_status'];
        //in 查询语句
...
//                        }
                ...
                if ($sql_in_val && $sql_between_val) {
                    ...
                    $filed_val = implode(' OR ',$sql_between_val);
                    $conditionData [$condition_id] ['condition_sql'] = '( '.$condition_sql.' OR '.$filed_val.' )';
...
//                $conditionData [$condition_id] ['condition_sql'] = str_replace(['$filedVal','$operator'],[$conditionData [$condition_id] ['filed_val'],$conditionData [$condition_id] ['operator']]  ,$conditionDatas [$conditionFieldKey]['condition_sql']);
    ...
    public static function getCheckedDatas($conditions)
    {
        ...
        $checkedData = [];
        //in 查询语句
        $inQueryTypeArr = [1,2,3,5,8,12,13,4,9,14];
//        //sku 邮编
//        $postCodeArr = [4,9];
        //sku 邮编
        //带运算符语句
...
//                    $condVal [1] = bcmul ($condVal [1],$rate,2);
                    $responseData ['filed_val'] = $condVal [0].' AND '.$condVal[1] ." AND orders.currency_code = '".strtoupper($data ['cond_unit'])."'";
                }
            } else {
                $responseData ['filed_val'] = $condVal [0].' AND '.$condVal[1];
            }
...
//                $rate = 6.7;
//                $condVal [1] = bcmul ($condVal [1],$rate,2);
                $responseData ['filed_val'] = $condVal [1]." AND orders.currency_code = '".strtoupper($data ['cond_unit'])."'";
            }
        } else {
            $responseData ['filed_val'] = $condVal[1];
        }
