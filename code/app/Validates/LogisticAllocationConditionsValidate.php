...
class LogisticAllocationConditionsValidate
{
    /**
     * @return array
     * Note: 订单问题规则
    ...
    public static function getAllocationConditionsRules($is_creat = false ,$trouble_id = 0)
    {
        ...
        if ($is_creat) {
            $rules ['trouble_rules_name'] = 'required|min:1|max:300|unique:rules_logistic_allocation|regex:/^[\x{4e00}-\x{9fa5}a-zA-Z0-9]+$/u';
        } else {
            $rules ['trouble_rules_name'] = 'required|min:1|max:300|unique:rules_logistic_allocation,trouble_rules_name,:id,id|regex:/^[\x{4e00}-\x{9fa5}a-zA-Z0-9]+$/u';
