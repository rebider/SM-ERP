...
function rakutenCategory(initialize = true, parentId = 0, level = 1, currentIndex = 0) {
    ...
    $.each(categoryNode, function (k, v) {
        rlAreaCont += '<li onClick="rakutenCategory( false,' + v.BrowseNodeID + ',' + v.categories_lv + ',' + k++ + ');" data-id="' + v.BrowseNodeID + '"><a href="javascript:void(0)">' + v.BrowseNodeName + '(' + v.BrowseNodeName_CN + ')' + '</a></li>';
    });
...
//初始化时不进行任何数据处理
    ...
    $("#rlselectedId").val('');
    sortClass.eq(currentIndex).each(function (k, v) {
        $("#rlselectedId").val($(v).attr('data-id'));
    });

    //设置最终选定的分类ID
    if (level == 3 || categoryNode.length <= 0) {
...
function getContent(parentId) {
    $.ajax({
        url: "/Goods/onlineAmazon/getCategory"
        , type: "get"
        , dataType: "json"
        , async: false
        , data: {
