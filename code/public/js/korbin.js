...
function pgout(url, value, parentId){

	//存储已打开过的tab页签
	let openedTab = null;
	//获取所有页签
	let menuTab = $("#menu-list a");
	//检查该次想要打开的页面是否已经打开过
	$.each(menuTab, function (key, tabValue) {
		if ($(this).text() == value) {
			openedTab = $(this);
		}
	});
	...
	if (!openedTab) {
		...
		var tab = $('<a href="javascript:void(0);" data-url="'+url+'" data-value="'+value+'" data-parent-id="'+ parentId +'" class="inlayer active">' + value + '<i class="menu-close"></i>' + '</a>');
