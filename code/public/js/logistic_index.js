...
//            var index = layer.msg('数据请求中', {icon: 16});
    ...
    $(document).on('click', '.probRulesBtn', function () {
        ...
        layer.open({
            ...
            area: ['1100px', '800px'],
            btn: ['确定', '取消'],
            content: 'logisticAllocationCreated',
    ...
    window.trouble_handle = function ($param, $action) {
        ...
        if ($action == 2) {
            ...
            layer.open({
                ...
                area: ['1100px', '800px'],
                btn: ['保存', '取消'],
                content: 'logisticAllocationDetail/' + $param + data,
                yes: function (index, layero) {
                    //点击确认触发 iframe 内容中的按钮提交
                    var submit = layero.find('iframe').contents().find("#formSubmit");
                    submit.click();
    ...
    window.del = function (id) {
        layer.confirm("确定删除？", function (e) {
            $.ajax({
                type: 'delete',
                url: 'logisticAllocationDelete/' + id,
                data: '',
                headers: {
            ...
            , cols: [[
                ...
                , {
                    field: 'trouble_rules_desc', width: 150, title: '规则描述', templet: function (d) {
                        var str = '';
                        if (d.rules_logistic_condition) {
                            layui.each(d.rules_logistic_condition, function (index, item) {
