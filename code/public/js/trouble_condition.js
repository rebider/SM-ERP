...
layui.use(['layer','form','element','laydate','table','formSelects'], function() {
    ...
    form.on('submit(formSubmit)', function(data){
        ...
        var li_length = $('#OrderRules .ruleSection .flexlayer').find('li');
        if (li_length.length == 0) {
            layer.msg('请配置规则', {icon: 5});
            return false;
        }
        li_length.each( function (){
                            ...
                            if ($(this).hasClass('alone')) {
                                ...
                                if (checked_val == 1) {
                                    ...
                                    if (goods_weight_value_max == '' || goods_weight_value_min == '') {
                                        except_status = true;
                                        except_mes = '最小值与最大值不能为空！请检查参数';
                                        return false;
                                    }

                                    goods_weight_value_max = parseFloat(goods_weight_value_max);
                                    goods_weight_value_min = parseFloat(goods_weight_value_min);
                                    if (goods_weight_value_max == 0 &&  goods_weight_value_min == 0) {
                                        except_status = true;
                                        except_mes = '最小值与最大值异常！请检查参数';
                                        return false;
                                    }
                    ...
                    $(layero).find('.proWeight .orders_price_class li').each(function(indexs){
                        ...
                        if ($(this).hasClass('alone')) {
                            ...
                            if (checked_val == 1) {
                                ...
                                orders_price_min = parseFloat(orders_price_min);
                                orders_price_max = parseFloat(orders_price_max);
                                if (orders_price_min == 0 && orders_price_max == 0) {
                                    except_status = true;
                                    except_mes = '最小值与最大值异常！请检查参数';
                                    return false;
                                }
        ...
        layer.open({
            ...
            yes: function(index, layero){
                ...
                if(checked_length == 0){
                    ...
                    var outs = '';
                    var set_val = kgross = unit_val = hiddenHtml = except_mes = '';
                    var except_status = false;
                    $(layero).find('.proWeight .goods_count_class input:radio:checked').each(function(){
                        ...
                        if (checkedVal == 1) {
                            ...
                            if (goods_count_max == '' || goods_count_min == '') {
                                except_status = true;
                                except_mes = '最小值或最大值不能为空';
                                return false;
                            }
                            goods_count_min = parseFloat(goods_count_min);
                            goods_count_max = parseFloat(goods_count_max);
                            if (goods_count_min == 0 && goods_count_max == 0) {
                                except_status = true;
                                except_mes = '最小值与最大值异常！请检查参数';
                                return false;
                            }

                            if (goods_count_min > goods_count_max) {
                                except_status = true;
                                except_mes = '最小值大于最大值！请检查参数';
                                return false;
                            }
