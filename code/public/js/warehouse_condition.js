        ...
        layer.open({
            ...
            yes: function(index, layero){
                if($(layero).find('.numTxtarea').val() == ''){
                    ...
                    var excludeZipCode = $(layero).find('.numTxtarea').val();
                    var result = "";
                    if (excludeZipCode != "") {
                        if (excludeZipCode.length > 5000) {
                            layer.msg('排除邮编总长度已超过上限，请检查后重新填写');
                            return false;
                        }
                        var excludeZipCodeArr = excludeZipCode.split("\n");
                        if (excludeZipCodeArr.length > 0) {
                            for (var i = 0; i < excludeZipCodeArr.length; i++) {
                                if (excludeZipCodeArr[i].indexOf("~") != -1) { //判断邮编范围左右值合法性
                                    var strArr = excludeZipCodeArr[i].trim().split("~");
                                    if (strArr.length != 2) {
