    ...
    var c = {
        ...
        createTabDom: function() {
            ...
            if (i._parentElem = e.elem, !i.tabDomExists()) {
                var a = ['<div class="layui-tab layui-tab-card kit-tab" lay-filter="' + i._filter + '">', '<ul class="layui-tab-title">', '<li class="layui-this" lay-id="-1"><i class="layui-icon">&#xe68e;</i> 首页</li>', "</ul>", '<div class="kit-tab-tool">操作&nbsp;<i class="layui-icon">&#xe625;</i></div>', '<div class="kit-tab-tool-body layui-anim layui-anim-upbit">', "<ul>", '<li class="kit-item" data-target="refresh">刷新当前选项卡</li>', '<li class="kit-line"></li>', '<li class="kit-item" data-target="closeCurrent">关闭当前选项卡</li>', '<li class="kit-item" data-target="closeOther">关闭其他选项卡</li>', '<li class="kit-line"></li>', '<li class="kit-item" data-target="closeAll">关闭所有选项卡</li>', "</ul>", "</div>", '<div class="layui-tab-content">', '<div class="layui-tab-item layui-show" lay-item-id="-1"><iframe class="iframesrc" src="' + e.mainUrl + '"></iframe></div>', "</div>", "</div>"];
        ...
        tabAdd: function(i) {
            ...
            if (t.tabExists(s)) t.tabChange(s);
                ...
                r.push("</li>");
                var o = '<div class="layui-tab-item layui-show" lay-item-id="' + s + '"><iframe class="iframesrc" src="' + c + '"></iframe></div>';
...
$(function(){		
	$('body').on('click','.cyn-new .layui-tab-close',function(){		
		var tid = $(this).parent('li');
		var index = tid.index();
		var fidindex = $('.layui-tab-content .layui-tab-item:eq('+index+')');		
		if(tid.hasClass('layui-this') && tid.next('li').length < 1){
			tid.prev('li').addClass('layui-this');
		}else if(tid.hasClass('layui-this') && tid.next('li').length >= 1){
			tid.next('li').addClass('layui-this');
		};
		if(fidindex.hasClass('layui-show') && fidindex.next('.layui-tab-item').length < 1){
			fidindex.prev('.layui-tab-item').addClass('layui-show');
			
		}else if(fidindex.hasClass('layui-show') && fidindex.next('.layui-tab-item').length >= 1){
			fidindex.next('.layui-tab-item').addClass('layui-show');
		};
		fidindex.remove();
		tid.remove();
