                            ...
                            <tr>
                                <td colspan="10">
                                    <div class="selectedSort firsttd"><b><i style="color: red;">*</i>亚马逊分类：</b>
                                        <i id="rlselectedSort" name="rlselectedSort" style="min-width:500px; width:auto!important;" class="inputi" style="width: 900px;margin-left: 10px;padding-left: 0px;">
                                ...
                                <td colspan="6">
                                    <div class="wareSort clearfix">
                                        @if(isset($goods['amazon_category_id']) && !empty($goods['amazon_category_id']))
                                            <ul id="rlsort1">
                                                @if (isset($categoryInfo ['category']['first']))
                                                    @foreach($categoryInfo ['category']['first'] as $key => $val)
                                                        <li
                                                                @if(in_array($val['BrowseNodeID'], $categoryInfo['categoryInArray']))
                                                                class="on"
                                                                @endif
                                            ...
                                            <ul id="rlsort2" @if (!isset($categoryInfo ['category']['second']))style="display: none"@endif>
                                                @if (isset($categoryInfo ['category']['second']))
                                                    @foreach($categoryInfo ['category']['second'] as $key => $val)
                                                        <li
                                                                @if(in_array($val['BrowseNodeID'], $categoryInfo['categoryInArray']))class="on"@endif
                                                        onclick="rakutenCategory(false, {{$val['BrowseNodeID']}}, {{$val['categories_lv']}}, {{$key}});"
                                                                data-id="{{$val['BrowseNodeID']}}"><a href="javascript:void(0)">{{$val['BrowseNodeName']}}({{$val ['BrowseNodeName_CN']}})</a></li>
                                                    @endforeach
                                                @endif
                                            </ul>
                                            <ul id="rlsort3" @if (!isset($categoryInfo ['category']['third']))style="display: none"@endif>
                    ...
                    <div class="produpage layui-form textr">
                        ...
                        <table class="layui-table shangjiainfor" lay-skin="nob">
                            ...
                            <tr>
                                ...
                                <td><input type="text" autocomplete="off" name="seller_sku" lay-verify="required" value="{{$goods['seller_sku']??''}}" /></td>
                                <td><b><i style="color: red;">*</i>商品编码：</b></td>
                                <td class="spe">
                                    <select name="product_code_type" lay-verify="required">
                                        <option value="">请选择</option>
                                        <option value="1" @if(!empty($goods['product_code_type']) && $goods['product_code_type'] == 1) selected @endif>UPC</option>
                                        <option value="2" @if(!empty($goods['product_code_type']) && $goods['product_code_type'] == 2) selected @endif>ASIN</option>
                                    </select>
                                    <input type="text" autocomplete="off" name="product_code"  value="{{$goods['product_code']??''}}" style="display: inline-block; width:300px"/>
                                </td>
                            ...
                            <tr>
                                ...
                                <td><input type="text" name="title" autocomplete="off" value="@if(!$id){{$goods['goods_title']??''}}@else{{$goods['title']??''}}@endif" /></td>
                                <td><b>关键词1：</b></td>
                    ...
                    <div class="produpage layui-form">
                        ...
                        <table class="layui-table shangpininfor" lay-skin="nob">
                            ...
                            <tr>
                                ...
                                <td colspan="5">
                                    <textarea name="goods_description" lay-verify="required"  placeholder="请输入文字"  class="layui-textarea">@if(!$id){{$goods['description']??''}}@else{{$goods['goods_description']??''}}@endif</textarea>
                                </td>
                            </tr>
                            </tbody>
                        </table>
                    ...
                    <div class="produpage layui-form">
                        ...
                        <table class="layui-table shangpininfor priceinfor" lay-skin="nob">
                            ...
                            <tr>
                                ...
                                <td>
                                    @if (isset($goods['platform_in_stock']) && isset($goods_quantity))
                                        <input name="platform_in_stock" type="number" min="1" max="99999999" autocomplete="off" lay-verify="required" value="{{empty($goods['platform_in_stock'])?$goods_quantity:$goods['platform_in_stock']}}" onkeyup="value=value.match(/\d+/,'')"/>
                                    @elseif (!isset($goods['platform_in_stock']) && isset($goods_quantity))
                                        <input type="number" min="1" max="99999999" autocomplete="off" lay-verify="required" name="platform_in_stock"  value="{{$goods_quantity}}" onkeyup="value=value.match(/\d+/,'')"/>
                                    @else
                                        <input type="number" min="1" max="99999999" autocomplete="off" lay-verify="required" name="platform_in_stock" value="" onkeyup="value=value.match(/\d+/,'')"/>
                                    @endif
                                </td>
                            </tr>
                            <tr>
                                ...
                                <td>
                                    <div class="layui-input-inline">
                                        <input type="text" autocomplete="off" name="promotion_time"
                                               @if (isset($goods['promotion_start_time']) && isset($goods['promotion_end_time']))
                                               @if(strtotime($goods['promotion_start_time']) <= 0  && strtotime($goods['promotion_end_time']) <= 0 )
                                               value ="" placeholder=" - "
                                               @else
                                               value="{{$goods['promotion_start_time'].' - '.$goods['promotion_end_time']}}"
                                               @endif
                                               value ="" placeholder=" - "
                                               @endif
                                               class="layui-input" id="test10" >
                                    </div>
                                </td>
                            </tr>
                            </tbody>
                                ...
                                <div class="layui-tab-item layui-show">
                                    <div class="layui-upload uploadoneimg">
                                        ...
                                        <div class="layui-upload-list">
                                            <div class="rlpicarea">
                                                @if($id)
                                                    <img onerror="this.src='/img/imgNotFound.jpg'" class="layui-upload-img" id="rldemo1" @if(!empty($goods['img_url'])) src="{{url('showImage').'?path='.$goods['img_url']}}" @else
                                                    {{--src="" --}}
                                                        @endif >
                                                    ...
                                                    @endif
                                                @else
                                                    <img  onerror="this.src='/img/imgNotFound.jpg'"class="layui-upload-img" id="rldemo1" @if(!empty($goods['goods_pictures'])) src="{{url('showImage').'?path='.(isset($goods['goods_pictures']) ? $goods['goods_pictures'] : '')}}" @else src="" @endif>
                                        ...
                                        <blockquote class="layui-elem-quote layui-quote-nm" style="margin-top: 10px;">
                                            ...
                                            <div class="layui-upload-list" id="rldemo2">
                                                @if(isset($goods['pictures']) && count($goods['pictures'])>0)
                                                    ...
                                                    {{--<span class="rldelet12" data-id="{{ $val['id'] }}"--}}
