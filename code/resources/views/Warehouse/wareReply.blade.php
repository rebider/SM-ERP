    ...
    <div class="sectionBody">
        <div class="location">
    		<span class="layui-breadcrumb">
			  <a href="">工单管理</a>
			  <a><cite>@if($priority == 1)待回复工单(普通)@elseif($priority == 2)待回复工单(重要)@elseif($priority == 3)待回复工单(紧急)@endif</cite></a>
			</span>
        </div>
        <div class="edn-row">
        ...
        layui.use(['layer','form','element','laydate','table'], function(){
            ...
            form.on('submit(searBtn)', function(data){
                ...
                table.render({
                    ...
                    ,where:{number:number,priority:priority}
                    ,cols: [[
                        ...
                        ,{fixed: 'right',  width:80,title:'操作',templet: function(d){
                            return '<a href="{{ url('?url=').'/Warehouse/ware_detail/' }}'+d.customer_complaint_id+'" target="_blank" class="layui-table-link">回复</a>'
                        }}
                    ]]
                    ,page: true
                    ,limits:[20,30,40,50]
